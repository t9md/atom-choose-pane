...
module.exports = async function choosePane(store) {
  ...
  for (const pane of allPanes) {
    ...
    if (pane === atom.workspace.getActivePane()) {
      className = "active"
    } else if (pane === store.lastFocused) {
      className = "last-focused"
    }
    atom.views.getView(pane).appendChild(createLabel(labelChar, className))
    paneByLabelChar[labelChar.toLowerCase()] = pane
  }
  ...
  if (action === "next-item") {
    ...
    const pane = action === "last-focused" ? store.lastFocused : paneByLabelChar[char.toLowerCase()]
    if (pane) {
      pane.activate()
    } else {
